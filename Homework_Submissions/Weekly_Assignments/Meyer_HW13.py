...
def get_my_streamflow(siteID, start_date, end_date):
       ...
       site ID. The data is then converted into a pandas dataframe. The
       dataframe is then parsed into a more usable format. The dataframe is
       then returned. 
...
def input_date(forecast_date):
       ...
       Forecast_date string is separated into month and day integers. The
       forecast date is then converted into a pandas dataframe. The dataframe
       is then parsed into a more usable format. The dataframe is then returned.
       
       Parameters
       ----------
       forecast_date : string 'YYYY-MM-DD'
              The forecast date entered by the user for the first day of the
              week 1 forecast.
              
       Returns
       -------
       integer values
              The forecast date start month and forecast date start day.
       '''
...
def get_my_1_week_dates(month, day):
       ...
       if (month == 1 or month == 3 or month == 5 or month == 7 or month == 8 or month == 10 or month == 12) & (stop_day > 31):
              ...
              stop_day2 = stop_day
              month2 = month
       return(month, day, month2, stop_day2)

week1_dates = np.array(get_my_1_week_dates(month, day))

def get_my_2_week_dates(month, day):
       '''
       2 Week Forecast Dates
       
       The 2 week forecast start month and day are attained from the input_date
       function. The end month and day are then calculated using the user
       forecast_date input. For forecast dates that end within the same month,
       the end month is the same as the start month and the end day is 14 days
       after the start day. For forecast dates that end within the next month,
       the end month is the next month and the difference is calculated between
       the end day and the number of days within the start month. The stop day
       is then equivalent to the aforementioned difference. The number of days
...
week2_dates = np.array(get_my_2_week_dates(month, day))
